...
import { initControls, updateControls, isPointerLocked, setYaw, } from './player/controls';
// ─── Renderer ────────────────────────────────────────────────────────────────
...
function drawMap() {
    ...
    function wx(x) { return 50 + (x + 500) / 1000 * 600; }
    function wz(z) { return 50 + (z + 350) / 500 * 400; }
    ctx.strokeStyle = '#c9a55c';
    ctx.lineWidth = 1;
    // Trade road
    ctx.beginPath();
    ctx.moveTo(wx(-500), wz(-92));
    ctx.lineTo(wx(500), wz(-92));
    ctx.globalAlpha = 0.4;
    ctx.stroke();
    ctx.globalAlpha = 1;
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(wx(-500), wz(-120));
    ctx.lineTo(wx(60), wz(-120));
    ctx.stroke();
    // Dashed underground reach
    ctx.setLineDash([6, 5]);
    ctx.beginPath();
    ctx.moveTo(wx(60), wz(-120));
    ctx.lineTo(wx(165), wz(-120));
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.beginPath();
    ctx.moveTo(wx(165), wz(-120));
    ctx.lineTo(wx(500), wz(-120));
    ctx.stroke();
    // Acropolis mound
    ctx.strokeStyle = '#c9a55c';
    ctx.lineWidth = 1.5;
