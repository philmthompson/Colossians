...
interface Region {
  name: string;
  subtitle: string;
  // Returns true if the player position is inside this region
  test: (pos: THREE.Vector3) => boolean;
}

const REGIONS: Region[] = [
  {
    name: 'The Lycus Valley',
    subtitle: 'Phrygia · Asia Minor',
    test: pos => pos.x < -120 && Math.abs(pos.z - (-92)) < 15,
  },
  {
    name: 'The Acropolis',
    subtitle: 'Höyük · Colossae',
    test: pos => {
      const dx = pos.x - 0, dz = pos.z - (-8);
      return Math.sqrt(dx * dx + dz * dz) < 50;
    },
  },
  {
    name: 'The Lower City',
    subtitle: 'Colossae · AD 52',
    test: pos => pos.x > 55 && pos.x < 195 && pos.z > -95 && pos.z < 0,
  },
  {
    name: 'The Theatre',
    subtitle: 'Cavea of Colossae',
    test: pos => {
  ...
  {
    name: 'The Necropolis',
    subtitle: 'North of the Lycus',
    test: pos => pos.x > -40 && pos.x < 100 && pos.z < -175 && pos.z > -230,
  },
  {
    name: 'The Chasm',
    subtitle: 'Herodotus 7.30',
    test: pos => pos.x > 55 && pos.x < 170 && Math.abs(pos.z - (-120)) < 22,
  },
  {
    name: 'Eastern Pastures',
    subtitle: 'The Flock of the Lycus',
    test: pos => {
...
export function updateRegions(pos: THREE.Vector3): void {
